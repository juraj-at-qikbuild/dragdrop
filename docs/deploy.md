...
```bash
npm test              # unit tests (vitest): protocol, server room, simulation
npm run e2e           # builds the client, starts a server, drives headless Chromium pages
npm run loadtest -- --bots 100 --duration 60     # bot clients against a running server
```
...
players join, and global caps set a hard ceiling. A governor also thins the city out when the average tick
time exceeds `TICK_BUDGET_MS`, so an overloaded Machine degrades gracefully instead of lagging. Check
`/stats` under real load. If `governor` stays well below 1 or `loop.p95` gets close to 50 ms, move up
to `shared-cpu-2x`/`performance-1x` in `fly.toml` (`[[vm]]`).
