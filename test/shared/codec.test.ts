...
describe('codec', () => {
  ...
  it('carries every level, including -1 (in a tunnel) and 2 (an upper deck), in STATE reports and entity records', () => {
    for (const lvl of [-1, 0, 1, 2] as const) {
      const w = new Writer(8);
    ...
    const at = w.n;
    w.u16(4);
    [-1, 0, 1, 2].forEach((level, i) => {
      const t = new Tram(null, null);
      t.id = 30 + i;
      t.sections = [{ x: 1, y: 2, a: 0 }, { x: 3, y: 4, a: 0 }, { x: 5, y: 6, a: 0 }];
      entityHead(w, t.id, Ent.Tram, false, level as -1 | 0 | 1 | 2);
      tramDynamic(w, t);
    });
    w.patchU16(at, 4);
    w.u16(0);
